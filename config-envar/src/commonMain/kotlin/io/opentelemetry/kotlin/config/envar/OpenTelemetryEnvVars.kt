...
class OpenTelemetryEnvVars(
    ...
    fun toBehavior(): OpenTelemetryBehavior = OpenTelemetryBehavior(
        ...
        loggerProvider = LoggerProviderBehavior(
            ...
            processor = toLogProcessorBehavior(),
        ),
    )

    private fun toLogProcessorBehavior(): LogRecordProcessorBehavior? {
        val exporter = LogsExporterEnvVars(reader, onWarning).toBehavior()
        return exporter?.let { LogRecordProcessorBehavior(exporter = it) }
    }
}

...
object DbAttributes {
    ...
    */
    ...
    /**
    * <p>The number of database operations included in a batch operation.</p>
    * <p>Notes:</p>
    * <p>Except for empty batch requests described below, a batch operation contains two
    * or more database operations explicitly submitted as separate operations in a single
    * client call, protocol message, or database command.</p>
    * <p>Requests to batch APIs that contain only one operation SHOULD be modeled as single
    * operations, not as batch operations.</p>
    * <p>A database call is not a batch operation solely because one operation accepts
    ...
    * multiple keys.</p>
    * <p>In batch APIs that execute the same parameterized operation with parameter sets,
    ...
    * as single operations, not as batch operations.</p>
    * <p><c>db.operation.batch.size</c> SHOULD be set to the number of operations in the batch.
    * It SHOULD NOT be set for non-batch operations.</p>
    * <p>A request to execute a batch operation with no operations SHOULD also be treated
    * as a batch operation, and <c>db.operation.batch.size</c> SHOULD be set to <c>0</c>.</p>
    */
